...
if (!is_file($configFile)) {
    gallery_upload_json(['status' => 'error', 'message' => 'SentryIQ configuration is unavailable.'], 503);
}
...
if ($dataDir === '' || !str_starts_with($dataDir, '/') || !is_dir($dataDir) || is_link($dataDir)) {
    gallery_upload_json(['status' => 'error', 'message' => 'SentryIQ secure runtime is unavailable.'], 503);
}
...
if (!is_array($files) || !isset($files['tmp_name'], $files['error'])) {
    gallery_upload_json(['status' => 'error', 'message' => 'No photos were supplied.'], 400);
}
$tmpNames = $files['tmp_name'];
$errors = $files['error'];
$names = $files['name'] ?? [];
if (!is_array($tmpNames) || !is_array($errors)) {
    gallery_upload_json(['status' => 'error', 'message' => 'Invalid photo upload data.'], 400);
}
