...
let selectedAlbumForDelete=null;
...
function clearSelection(){
  ...
  document.querySelectorAll('.gallery-filter').forEach(function(button){button.classList.remove('album-selected');});
  updateSelectionUi();
}
...
document.querySelectorAll('.gallery-filter').forEach(function(button){
  button.addEventListener('click',function(){
    ...
    if(this.classList.contains('active') && selectedAlbumForDelete===null){
      selectedAlbumForDelete=album;
      visiblePhotoCards().forEach(function(card){
        const checkbox=card.querySelector('.gallery-photo-check');
        if(checkbox)checkbox.checked=true;
      });
    ...
    if(selectedAlbumForDelete===album){
      selectedAlbumForDelete=null;
      document.querySelectorAll('.gallery-photo-check').forEach(function(checkbox){checkbox.checked=false;});
      this.classList.remove('album-selected');
      updateSelectionUi();
      return;
    }
    applyFilter(album);
