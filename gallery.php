...
function uploadGalleryPhoto(form,file,onProgress){return new Promise(async function(resolve,reject){
  if(!(file instanceof File)||file.size<=0){reject(new Error('CLIENT_UPLOAD_DIAGNOSTIC: Invalid File object before upload.'));return;}
  const uploadData=new FormData();
  uploadData.append('csrf_token',csrf);
  uploadData.append('photos[]',file,file.name);
  const formDataPhoto=uploadData.get('photos[]');
  const clientDiagnostics={file_name:file.name,file_size:file.size,file_type:file.type,file_is_file:file instanceof File,form_data_photo_is_file:formDataPhoto instanceof File,form_data_photo_name:formDataPhoto instanceof File?formDataPhoto.name:'',form_data_photo_size:formDataPhoto instanceof File?formDataPhoto.size:0};
  if(!(formDataPhoto instanceof File)||formDataPhoto.size!==file.size){reject(new Error('CLIENT_UPLOAD_DIAGNOSTIC: FormData file mismatch: '+JSON.stringify(clientDiagnostics)));return;}
  if(typeof onProgress==='function')onProgress(8);
  try{
    const response=await fetch(form.action,{method:'POST',body:uploadData,credentials:'same-origin',headers:{Accept:'application/json','X-CSRF-Token':csrf,'X-SentryIQ-Client-Diagnostics':btoa(unescape(encodeURIComponent(JSON.stringify(clientDiagnostics))))}});
    if(typeof onProgress==='function')onProgress(92);
    const textResponse=await response.text();
    let data;
    try{data=JSON.parse(textResponse);}catch(_){reject(new Error('The upload returned an invalid response (HTTP '+response.status+'): '+textResponse.slice(0,300)));return;}
    if(!response.ok||!Array.isArray(data.results)){
      let detail=data.message||('Upload failed (HTTP '+response.status+').');
      if(data.request_diagnostics)detail+=' Diagnostics: '+JSON.stringify(data.request_diagnostics);
      reject(new Error(detail));return;
    }
    if(typeof onProgress==='function')onProgress(100);
    resolve(data);
  }catch(error){
    reject(error instanceof Error?error:new Error('Network error while uploading the photo.'));
  }
});}
