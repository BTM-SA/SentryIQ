...
@media (max-width: 600px){.gallery-album-form{flex-direction:column;align-items:stretch;max-width:none}.gallery-album-form input,.gallery-album-form button{width:100%;box-sizing:border-box}.gallery-album-form button{margin-top:2px}}
</style>
</head>
<body><div class="box">
...
<div class="gallery-upload"><h3 style="margin-top:0;">Upload Photos</h3><form id="gallery-upload-form" method="POST" action="gallery_upload.php" enctype="multipart/form-data"><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf, ENT_QUOTES, 'UTF-8'); ?>"><input type="file" name="photos[]" accept="image/jpeg,image/png,image/gif,image/webp" multiple required><button type="submit" class="btn btn-primary">Upload Photos</button></form><div id="gallery-message" class="gallery-message" aria-live="polite"></div></div>
