...
function save_passwords(array $records): bool
{
    ...
    if (!is_string($masterKey) || strlen($masterKey) !== 32) { vault_action_diagnostic('VAULT_SAVE_FAILURE', ['reason'=>'master_key_unavailable','record_count'=>count($records)]); return false; }
    try {
        ...
        if (!$saved) { vault_action_diagnostic('VAULT_SAVE_FAILURE', ['reason'=>'vault_write_encrypted_records_returned_false','record_count'=>count($records)]); return false; }
        vault_action_diagnostic('VAULT_SAVE_COMPLETED', ['record_count'=>count($records)]); return true;
        ...
        error_log('SentryIQ vault save failure: ' . $exception::class . ': ' . $exception->getMessage()); return false;
    }
}
...
if ($action === 'add_category') {
    ...
    if ($category === '' || strlen($category) > 50 || !preg_match('/^[\p{L}\p{N}][\p{L}\p{N} ._&()\-]{0,49}$/u', $category)) {
        http_response_code(422); header('Content-Type: application/json; charset=utf-8'); echo json_encode(['status'=>'error','message'=>'Please enter a valid category name.']); exit;
    }
    ...
    foreach ($passwords as $index => $entry) {
        ...
        foreach ($categories as $existingCategory) {
            $existingKey = function_exists('mb_strtolower') ? mb_strtolower($existingCategory, 'UTF-8') : strtolower($existingCategory);
            if ($existingKey === $categoryKey) { http_response_code(409); header('Content-Type: application/json; charset=utf-8'); echo json_encode(['status'=>'error','message'=>'That category already exists.']); exit; }
        }
        $categories[] = $category;
        natcasesort($categories); $categories = array_values($categories);
        $passwords[$index]['categories'] = $categories;
        break;
    }
    if (!$foundConfig) $passwords[] = ['id'=>'sys_config_node','type'=>'system_config','categories'=>[$category]];
    if (!save_passwords($passwords)) { http_response_code(500); header('Content-Type: application/json; charset=utf-8'); echo json_encode(['status'=>'error','message'=>'Unable to save the category.']); exit; }
    log_security_event('VAULT_CATEGORY_CREATED', get_visitor_ip(), $_SESSION['app_username'] ?? 'unknown', ['category'=>$category]);
    header('Content-Type: application/json; charset=utf-8'); echo json_encode(['status'=>'ok','category'=>$category]); exit;
}
...
if ($action === 'edit') {
    ...
    log_security_event('VAULT_RECORD_UPDATED', get_visitor_ip(), $_SESSION['app_username'] ?? 'unknown', ['entry_id'=>$entryId]); header('Location: index.php?status=updated&pane=view'); exit;
}

if ($action === 'delete') {
    $entryId=trim((string)($_POST['entry_id'] ?? '')); if ($entryId==='') { header('Location: index.php?status=error&pane=view'); exit; }
    $found=false; foreach ($passwords as $index=>$item) { if (($item['id'] ?? '') !== $entryId) continue; $found=true; $icon=(string)($item['icon_path'] ?? ''); if ($icon!=='' && is_file($icon) && str_starts_with($icon,SENTRYIQ_DATA_DIR.'/vault_icons/')) @unlink($icon); unset($passwords[$index]); break; }
    if (!$found || !save_passwords(array_values($passwords))) { header('Location: index.php?status=error&pane=view'); exit; }
    log_security_event('VAULT_RECORD_DELETED',get_visitor_ip(),$_SESSION['app_username'] ?? 'unknown',['entry_id'=>$entryId]); header('Location: index.php?status=deleted&pane=view'); exit;
}
...
if ($action === 'save_settings') {
    ...
    if (
        !filter_var($requestedBaseUrl, FILTER_VALIDATE_URL) ||
        !is_array($baseParts) ||
    ...
    if ($requestedDirectory !== '' && $requestedDirectory !== SENTRYIQ_DATA_DIR) {
        ...
        if (!$trustedRequestedDirectory && !is_dir($requestedDirectory)) {
            if (!@mkdir($requestedDirectory, 0700, true)) {
                header('Location: index.php?status=error&pane=settings');
                exit;
            }
        ...
        if (!@copy(DATA_FILE, $newDir . '/passwords.enc')) {
            $ok = false;
        }

        if (is_file(LOG_FILE) && !@copy(LOG_FILE, $newDir . '/security_audit.log')) {
            $ok = false;
        }

        if (defined('DIAGNOSTIC_LOG_FILE') && is_file(DIAGNOSTIC_LOG_FILE) && !@copy(DIAGNOSTIC_LOG_FILE, $newDir . '/diagnostic.log')) {
            $ok = false;
        }

        foreach (['vault_engine.php', 'email_template.php', 'vault_icon_cache.php'] as $runtimeFile) {
            if (!@copy(SENTRYIQ_DATA_DIR . '/' . $runtimeFile, $newDir . '/' . $runtimeFile)) {
                $ok = false;
            }
        }
        ...
        if (is_dir($oldIconDir)) {
            if (!is_dir($newIconDir) && !@mkdir($newIconDir, 0700, true)) {
                $ok = false;
            }

            if (is_dir($newIconDir)) {
        ...
        } elseif (!is_dir($newIconDir) && !@mkdir($newIconDir, 0700, true)) {
            $ok = false;
        }

        if (
            !$ok ||
            !is_file($newDir . '/passwords.enc') ||
            !is_file($newDir . '/vault_engine.php') ||
            !is_file($newDir . '/email_template.php') ||
            !is_file($newDir . '/vault_icon_cache.php')
        ) {
            header('Location: index.php?status=error&pane=settings');
            exit;
        }
        ...
        if (is_file($newDir . '/security_audit.log')) {
            @chmod($newDir . '/security_audit.log', 0600);
        }
        if (is_file($newDir . '/diagnostic.log')) {
            @chmod($newDir . '/diagnostic.log', 0600);
        }
        @chmod($newDir . '/vault_engine.php', 0600);
        @chmod($newDir . '/email_template.php', 0600);
        @chmod($newDir . '/vault_icon_cache.php', 0600);
    ...
    foreach ($passwords as $index => $entry) {
        ...
        if ($imapPassword !== '') {
            $passwords[$index]['imap_password'] = $imapPassword;
        }
        $updated = true;
        break;
    }
    ...
    $pointer = "<?php\nreturn [\n    'data_dir' => " . var_export($effectiveDirectory, true) . ",\n    'base_url' => " . var_export($requestedBaseUrl, true) . ",\n];\n";
    $tmp = dirname(__DIR__, 2) . '/sentryiq_config.php.tmp-' . bin2hex(random_bytes(8));
    ...
    if ($movedDirectory) {
        log_security_event('DATA_DIRECTORY_CHANGED', get_visitor_ip(), $username, [
            'from' => SENTRYIQ_DATA_DIR,
            'to' => $effectiveDirectory,
        ]);
    }
