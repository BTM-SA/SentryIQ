...
if ($action === 'rename_folder') {
    ...
    foreach ($passwords as $index => $entry) {
        ...
        $found = false;
        foreach ($existingFolders as $existingFolder) if (vault_name_key(trim($existingFolder)) === $newFolderKey && vault_name_key(trim($existingFolder)) !== $folderKey) redirect_category('error', $category);
        ...
        foreach ($passwords as $recordIndex => $record) {
            ...
            if (vault_name_key(trim((string)($record['category'] ?? ''))) === vault_name_key($storedCategory) && vault_name_key(trim((string)($record['folder'] ?? ''))) === $folderKey) $passwords[$recordIndex]['folder'] = $newFolder;
        }
        if (!save_category_vault($passwords)) redirect_category('error', $category);
...
if ($action === 'rename_category') {
    ...
    foreach ($passwords as $index => $entry) {
        ...
        foreach ($categories as $existingCategory) if (vault_name_key(trim($existingCategory)) === $newCategoryKey && vault_name_key(trim($existingCategory)) !== $categoryKey) redirect_category('error');
