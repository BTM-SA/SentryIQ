...
function normalize_vault_records(array $records):array{
 ...
 foreach($records as $record){
  if(is_array($record)){
   ...
   if(array_key_exists('label',$record)||array_key_exists('id',$record)&&!in_array(0,$keys,true)){$normalized[]=$record;continue;}
   if($keys===range(0,count($record)-1)&&count($record)>=6){$normalized[]=['id'=>trim((string)$record[5]),'label'=>trim((string)$record[0]),'username'=>trim((string)$record[1]),'password'=>trim((string)$record[2]),'url'=>trim((string)$record[3]),'notes'=>trim((string)$record[4]),'created_at'=>null,'updated_at'=>null,'icon_type'=>null,'icon_path'=>null,'icon_source'=>null,'icon_fetched_at'=>null];$changed=true;continue;}
   $normalized[]=$record;continue;
  }
  if(!is_string($record)){$normalized[]=$record;continue;}
  $parts=str_getcsv($record,',','"','\\');if(count($parts)<6){$normalized[]=$record;continue;}
  $normalized[]=['id'=>trim((string)$parts[5]),'label'=>trim((string)$parts[0]),'username'=>trim((string)$parts[1]),'password'=>trim((string)$parts[2]),'url'=>trim((string)$parts[3]),'notes'=>trim((string)$parts[4]),'created_at'=>null,'updated_at'=>null,'icon_type'=>null,'icon_path'=>null,'icon_source'=>null,'icon_fetched_at'=>null];$changed=true;
 }
 return[$normalized,$changed];
}
function load_passwords(?string $explicit_key=null):array|bool{$master_key=$explicit_key??($_SESSION['master_key']??null);if(!$master_key||!file_exists(DATA_FILE))return $explicit_key?false:[];$raw=@file_get_contents(DATA_FILE);if(empty($raw))return[];$payload=json_decode($raw,true);if(!$payload||!isset($payload['ciphertext'],$payload['iv'],$payload['tag']))return false;$decrypted=openssl_decrypt(base64_decode($payload['ciphertext']),'aes-256-gcm',$master_key,OPENSSL_RAW_DATA,base64_decode($payload['iv']),base64_decode($payload['tag']));if($decrypted===false)return false;$records=json_decode($decrypted,true);if(!is_array($records))return[];[$normalized,$changed]=normalize_vault_records($records);if($changed&&$explicit_key===null&&isset($_SESSION['master_key']))save_passwords($normalized);return $normalized;}
