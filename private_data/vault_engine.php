...
function cleanup_expired_tokens(): void {
    ...
    foreach (glob(SENTRYIQ_DATA_DIR . '/token_*.json') ?: [] as $tokenFile) {
        ...
        if (!is_array($token) || empty($token['expires']) || (int)$token['expires'] <= time()) @unlink($tokenFile);
    }
}

function log_security_event(string $event_type, string $ip_address, ?string $username = null, array $context = []): void {
    if (!ensure_sentryiq_data_directory()) return;
    $entry = ['timestamp'=>date('c'),'event'=>$event_type,'username'=>$username ?? ($_SESSION['app_username'] ?? 'unknown'),'ip'=>$ip_address,'user_agent'=>$_SERVER['HTTP_USER_AGENT'] ?? 'unknown','session_id'=>session_id() ?: null,'context'=>$context];
...
function read_security_log(): array {
    ...
    if (!$lines) return [];
    $events=[]; foreach(array_reverse($lines) as $line){$decoded=json_decode($line,true);if(is_array($decoded))$events[]=$decoded;} return $events;
}
...
function load_passwords(?string $explicit_key=null): array|bool {
    ...
    $raw=@file_get_contents(DATA_FILE);if(empty($raw))return [];
    $payload=json_decode($raw,true);if(!$payload||!isset($payload['ciphertext'],$payload['iv'],$payload['tag']))return false;
    ...
    $records=json_decode($decrypted,true);if(!is_array($records))return [];
    [$normalized,$changed]=normalize_vault_records($records);if($changed&&$explicit_key===null&&isset($_SESSION['master_key']))save_passwords($normalized);return $normalized;
}

function save_passwords(array $data_matrix): bool {
    $master_key=$_SESSION['master_key']??null;if(!$master_key||!ensure_sentryiq_data_directory())return false;
    $iv=openssl_random_pseudo_bytes(openssl_cipher_iv_length('aes-256-gcm'));$ciphertext=openssl_encrypt(json_encode($data_matrix),'aes-256-gcm',$master_key,OPENSSL_RAW_DATA,$iv,$tag);
